...
fn day_04_squid_bingo() {
    ...
    let draws: Vec<u32> = raw_lines[0].split(",").map(|x| u32::from_str(x).unwrap()).collect();
    ...
    for board_idx in 0..n_boards {
        ...
        for row_idx in 0..5 {
            ...
            let row_vals: Vec<u32> = raw_row.split(" ").map(
                |x| x.trim()
            ).filter(|x| x.len() > 0).map(|x| u32::from_str(x).unwrap()).collect();
            // TODO(andrei): Can we do this parsing in a cleaner way?
            for col_idx in 0..5 {
                values[row_idx][col_idx] = row_vals[col_idx];
            }
        }
        let mut board = Board{values: values, seen: seen};
        boards.push(board);
    }
