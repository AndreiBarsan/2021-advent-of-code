...
fn bool_to_pixel(val: &bool) -> char {
    if *val { LIGHT_PIXEL } else { DARK_PIXEL }
}
...
fn pixel_to_bool(pixel: char) -> bool {
    if pixel == LIGHT_PIXEL {
        ...
        panic!("Invalid color character: {} (code = {})", pixel, pixel as u32);
    }
}
...
fn conv2d_world(world: &World, lookup: &Vec<bool>, kernel_size: usize) -> World {
        ...
        for out_col in 0..out_image[0].len() {
            ...
            for row_off in -off..=off {
                for col_off in -off..=off {
                    ...
                    let val = if row_in >= 0 && row_in < (n_in_rows as i32) && col_in >= 0 && col_in < (n_in_cols as i32) {
    ...
    World { finite_map: out_image, background: new_background }
}
...
fn conv2d_with_lookup(base_image: &Vec<Vec<bool>>, lookup: &Vec<bool>, kernel_size: usize) -> Vec<Vec<bool>> {
    if kernel_size % 2 == 0 {
        panic!("Kernel size must be odd");
    }
...
fn day_20_trench_map() {
    ...
    let mut cur_world = World { finite_map: initial_image.clone(), background: false };
